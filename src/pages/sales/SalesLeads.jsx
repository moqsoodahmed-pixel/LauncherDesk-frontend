...
const STATUS_COLORS = { new:'#3B5BDB',working:'#0EA5E9',nurturing:'#F59E0B',converted:'#10B981',lost:'#EF4444' }
...
export default function SalesLeads() {
  ...
  const [edit, setEdit] = useState({ status:'', notes:'', assignedTo:'', followUpDate:'' })
  const PER = 20
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (q) p.set('q', q)
      const r = await apiFetch(`/sales/leads?${p}`)
      setLeads(r.data || []); setTotal(r.total || 0)
  ...
  const open = l => { setSelected(l); setEdit({ status:l.status, notes:l.notes||'', assignedTo:l.assignedTo?._id||'', followUpDate:fmtDate(l.followUpDate) }) }

  const save = async () => {
    setSaving(true)
    try {
      const updated = await apiFetch(`/sales/leads/${selected._id}`, { method:'PATCH', body:JSON.stringify(edit) })
  ...
  return (
    <div style={{maxWidth:1100,margin:'0 auto'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20,flexWrap:'wrap',gap:12}}>
        <div>
          <h1 style={{fontSize:22,fontWeight:800,color:'#0A2540'}}>Leads</h1>
          <p style={{color:'#64748B',fontSize:13,marginTop:2}}>{total} total leads</p>
        </div>
        <button onClick={load} style={{padding:'8px 16px',background:'#fff',border:'1.5px solid #E2E8F0',borderRadius:8,fontSize:13,fontWeight:600,cursor:'pointer',fontFamily:'inherit',color:'#475569'}}>
          ↻ Refresh
        </button>
      </div>
          ...
          <table style={{width:'100%',borderCollapse:'collapse',minWidth:600}}>
            <thead>
              <tr style={{background:'#F8FAFC'}}>
                {['Name','Mobile','Service','State','Status','Follow-Up','Action'].map(h=>(
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7} style={{padding:'40px',textAlign:'center',color:'#94A3B8'}}>No leads found</td></tr>
              ) : leads.map(l => (
                <tr key={l._id} style={{borderTop:'1px solid #F1F5F9',transition:'background .1s'}} onMouseEnter={e=>e.currentTarget.style.background='#FAFBFF'} onMouseLeave={e=>e.currentTarget.style.background=''}>
                  <td style={{padding:'12px 16px'}}>
                    <div style={{fontWeight:600,fontSize:13.5,color:'#0A2540'}}>{l.name}</div>
                    <div style={{fontSize:12,color:'#64748B'}}>{l.email}</div>
                  </td>
                  <td style={{padding:'12px 16px'}}>
                    <a href={`tel:${l.mobile}`} style={{fontSize:13,color:'#3B5BDB',fontWeight:600,textDecoration:'none'}}>{l.mobile||'—'}</a>
                  </td>
                  <td style={{padding:'12px 16px',fontSize:12.5,color:'#475569',maxWidth:160}}><div style={{overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{l.serviceInterest||'—'}</div></td>
                  <td style={{padding:'12px 16px',fontSize:12.5,color:'#475569'}}>{l.state||'—'}</td>
                  <td style={{padding:'12px 16px'}}><Badge s={l.status}/></td>
                  <td style={{padding:'12px 16px',fontSize:12,color:l.followUpDate?'#F59E0B':'#94A3B8'}}>{fmt(l.followUpDate)}</td>
                  <td style={{padding:'12px 16px'}}>
                    <div style={{display:'flex',gap:6}}>
                      <button onClick={()=>open(l)} style={{padding:'5px 12px',background:'#EEF2FF',color:'#3B5BDB',border:'none',borderRadius:6,fontSize:12,fontWeight:600,cursor:'pointer',fontFamily:'inherit'}}>
                        Update
                      </button>
                      {l.mobile && (
                        <a href={`https://wa.me/91${l.mobile.replace(/\D/g,'')}`} target="_blank" rel="noopener noreferrer"
                          style={{padding:'5px 10px',background:'#DCFCE7',color:'#16A34A',borderRadius:6,fontSize:12,fontWeight:600,textDecoration:'none'}}>
                          WA
                        </a>
                      )}
                    </div>
        ...
        {total > PER && (
          <div style={{padding:'12px 16px',borderTop:'1px solid #F1F5F9',display:'flex',gap:8,justifyContent:'center'}}>
            {Array.from({length:Math.ceil(total/PER)},(_,i)=>i+1).map(p=>(
              <button key={p} onClick={()=>setPage(p)} style={{width:32,height:32,borderRadius:6,border:'1.5px solid',borderColor:p===page?'#3B5BDB':'#E2E8F0',background:p===page?'#EEF2FF':'#fff',color:p===page?'#3B5BDB':'#475569',fontSize:13,fontWeight:600,cursor:'pointer',fontFamily:'inherit'}}>
                {p}
              </button>
            ))}
          </div>
          ...
          <div style={{background:'#fff',borderRadius:18,width:'100%',maxWidth:520,maxHeight:'90vh',overflow:'auto',boxShadow:'0 24px 80px rgba(0,0,0,.25)'}}>
            <div style={{padding:'20px 24px',borderBottom:'1px solid #F1F5F9',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              <div>
                ...
                <div style={{fontSize:13,color:'#64748B'}}>{selected.email} · {selected.mobile}</div>
              </div>
            ...
            <div style={{padding:'20px 24px',display:'flex',flexDirection:'column',gap:14}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,background:'#F8FAFC',borderRadius:10,padding:14}}>
                {[['Service',selected.serviceInterest||'—'],['Business',selected.businessType||'—'],['State',selected.state||'—'],['Source',selected.source||'—']].map(([k,v])=>(
